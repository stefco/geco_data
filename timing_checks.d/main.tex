...
\title[Timing Witness Paper for LVC \graceId]{Timing Witness Signals Indicate Trusty Timing 
for LVC \graceId, a Burst Event Candidate}
...
The following 12 pages (figures 5-16) show the zero crossing region of the second-to-second average of the DuoTone witness signals around the second edge, zoomed-in at different magnifications in the x-axis. When the DuoTone signals are symmetric around the 0V level, the zero crossing should be delayed compared to the second tic of the datastream by $\sim 63 \mu s$ ($6.7 \mu s$ of this is due to an inherent delay on the timing Slave-DuoTone stack (see \href{https://dcc.ligo.org/LIGO-T1500513}{LIGO-T1500513}), and the rest is due to $65536Hz$ to $16384Hz$ decimation filter (see appendix to \href{https://dcc.ligo.org/LIGO-T1500516}{LIGO-T1500516}).

On the figures the open circles reflect the average signal, the green error bars indicate the standard deviation, and the ends of the fine black error bars show the maximum/minimum for each data point. The line through the data points guide the eye to help visualize the zero crossing, which is most visible at the medium timescale plotted, and is at a bit above $63 \mu s$. The plots indicate precise agreement with the expected place of the zero crossing and confirm the independent verification measurement by LHO and LLO rapid response team discussed in Section 6 of this document.
...
\centering
\caption{Best-fit piecewise linear curve in the zero crossing region of the second-to-second average of the DuoTone witness signals around the second edge at Hanford EX for the hour surrounding the \graceId\ event candidate. The zero crossing is quite visible at this magnification and is at $\sim 63.3 \mu s$, out of which $62.6 \mu s = 6.7 \mu s$ (DuoTone generation delay) $+ 55.9 \mu s$ (decimation filter delay) of the observed delay is accounted for. This naive zero-crossing approach \textbf{does not} account for DC shift of the signal, which would place the zero-crossing away from zero-phase.}
\centering
\end{figure}
...
\centering
\caption{Best-fit piecewise linear curve in the zero crossing region of the second-to-second average of the DuoTone witness signals around the second edge at Hanford EY for the hour surrounding the \graceId\ event candidate. The zero crossing is quite visible at this magnification and is at $\sim 63.3 \mu s$, out of which $62.6 \mu s = 6.7 \mu s$ (DuoTone generation delay) $+ 55.9 \mu s$ (decimation filter delay) of the observed delay is accounted for. This naive zero-crossing approach \textbf{does not} account for DC shift of the signal, which would place the zero-crossing away from zero-phase.}
\centering
\end{figure}
...
\centering
\caption{Best-fit piecewise linear curve in the zero crossing region of the second-to-second average of the DuoTone witness signals around the second edge at Livingston EX for the hour surrounding the \graceId\ event candidate. The zero crossing is quite visible at this magnification and is at $\sim 63.3 \mu s$, out of which $62.6 \mu s = 6.7 \mu s$ (DuoTone generation delay) $+ 55.9 \mu s$ (decimation filter delay) of the observed delay is accounted for. This naive zero-crossing approach \textbf{does not} account for DC shift of the signal, which would place the zero-crossing away from zero-phase.}
\centering
\end{figure}
...
\centering
\caption{Best-fit piecewise linear curve in the zero crossing region of the second-to-second average of the DuoTone witness signals around the second edge at Livingston EY for the hour surrounding the \graceId\ event candidate. The zero crossing is quite visible at this magnification and is at $\sim 63.3 \mu s$, out of which $62.6 \mu s = 6.7 \mu s$ (DuoTone generation delay) $+ 55.9 \mu s$ (decimation filter delay) of the observed delay is accounted for. This naive zero-crossing approach \textbf{does not} account for DC shift of the signal, which would place the zero-crossing away from zero-phase.}
\centering
\end{figure}
